...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect("login")  # Replace 'success' with the desired URL or view name
    else:
        form = RegisterForm()
...
def return_book(request, id):
    borrowedBook = BorrowedBook.objects.filter(id=id).first()
    borrowedBook.book.is_borrowed=False
...
def reports(request):
    ...
    return render(request, 'reports.html', {'overdue_books': overdue_books})
